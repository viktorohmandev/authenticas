...
// Install shared dependencies first (include dev for TypeScript types)
console.log('📦 Installing shared dependencies...');
execSync('npm install --include=dev', { cwd: path.join(ROOT, 'shared'), stdio: 'inherit' });

// Build Admin
console.log('🔨 Building Admin dashboard...');
execSync('npm install --include=dev', { cwd: path.join(ROOT, 'admin'), stdio: 'inherit' });
execSync('npm run build', { cwd: path.join(ROOT, 'admin'), stdio: 'inherit' });

// Build Retailer
console.log('🔨 Building Retailer dashboard...');
execSync('npm install --include=dev', { cwd: path.join(ROOT, 'retailer'), stdio: 'inherit' });
execSync('npm run build', { cwd: path.join(ROOT, 'retailer'), stdio: 'inherit' });

// Build Company
console.log('🔨 Building Company dashboard...');
execSync('npm install --include=dev', { cwd: path.join(ROOT, 'company'), stdio: 'inherit' });
